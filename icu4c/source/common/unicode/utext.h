 ...
 *               zero terminated.
 * @param capacity The number of UChars in the buffer (sizeof), or -1 if the buffer size
 *               is unknown and length is used.
 ...
 */
U_STABLE UText * U_EXPORT2
utext_openConstU8(UText *ut, const uint8_t *s, int64_t length, int64_t capacity, UErrorCode *status);

/**
 * Open a read/write UText implementation for UTF-8 strings.
 *
 * \htmlonly
 * Any invalid UTF-8 in the input will be handled in this way:
 * a sequence of bytes that has the form of a truncated, but otherwise valid,
 ...
 * @param length The length of the UTF-8 string in bytes, or -1 if the string is
 *               zero terminated.
 * @param capacity The number of UChars in the buffer (sizeof). Must be >= 0.
 * @param status Errors are returned here.
 * @return       A pointer to the UText.  If a pre-allocated UText was provided, it
 *               will always be used and returned.
 * @stable
 */
U_STABLE UText * U_EXPORT2
utext_openU8(UText *ut, uint8_t *s, int64_t length, int64_t capacity, UErrorCode *status);
 ...
 */
U_STABLE UText * U_EXPORT2
utext_openConstU16(UText *ut, const UChar *s, int64_t length, int64_t capacity, UErrorCode *status);

/**
 * Open a read/write UText for UChar * string.
 *
 * @param ut     Pointer to a UText struct.  If NULL, a new UText will be created.
 *               If non-NULL, must refer to an initialized UText struct, which will then
 *               be reset to reference the specified UChar string.
 ...
 */
U_STABLE UText * U_EXPORT2
utext_openU16(UText *ut, UChar *s, int64_t length, int64_t capacity, UErrorCode *status);
...
/**
 * Open a read-only UText for UChar32 * string.
 *
 * @param ut     Pointer to a UText struct.  If NULL, a new UText will be created.
 *               If non-NULL, must refer to an initialized UText struct, which will then
 *               be reset to reference the specified UChar string.
 * @param s      A UChar32 (UTF-32) string
 * @param length The number of UChars in the input string, or -1 if the string is
 *               zero terminated.
 * @param capacity The number of UChars in the buffer (sizeof), or -1 if the buffer size
 *               is unknown and length is used.
 ...
 */
U_STABLE UText * U_EXPORT2
utext_openConstU32(UText *ut, const UChar32 *s, int64_t length, int64_t capacity, UErrorCode *status);

/**
 * Open a read/write UText for UChar32 * string.
 *
 * @param ut     Pointer to a UText struct.  If NULL, a new UText will be created.
 *               If non-NULL, must refer to an initialized UText struct, which will then
 *               be reset to reference the specified UChar string.
 ...
 */
U_STABLE UText * U_EXPORT2
utext_openU32(UText *ut, UChar32 *s, int64_t length, int64_t capacity, UErrorCode *status);
...
/**
 *  Common function to determine if a UText is valid for use, or if there is an issue
 *  with the UText struct.
 *
 * @param ut   pointer to a UText struct to be re-used, or null if a new UText
 *             should be allocated.
 * @return TRUE if the UText struct is valid.
 * @stable
 */
...
/**
 *  Common function for use by UText Provider implementations to allocate and/or initialize
...
/**
 *  Common function for use by UText Provider implementations to allocate and/or initialize
 *  a shallow UText struct clone. To be called in the implementation of utext_clone() functions.
 *
 * @param dest pointer to a UText struct to be cloned
 * @param dest pointer to a UText struct to be re-used, or null if a new UText
 *             should be allocated.
 * @param status Errors are returned here.
 * @return pointer to the UText, allocated if necessary, with extra space set up if requested.
 * @stable ICU 3.4
